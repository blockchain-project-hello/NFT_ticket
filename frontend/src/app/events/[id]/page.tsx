...
export default function EventDetail({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    getEvent(params.id).then(setEvent).catch(() => setEvent(null));
  }, [params.id]);
  ...
  const handleMint = () => {
    ...
    try {
      writeContract({
        ...
        args: [BigInt(event.blockchain_event_id)],
        value: BigInt(event.base_price_wei),
      });
    } catch (err) {
  ...
  const progressPercentage = (event.tickets_minted / event.total_supply) * 100;
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12 w-full animate-fadeIn">
      ...
      <div className="relative rounded-3xl overflow-hidden h-[400px] mb-12 shadow-2xl">
        ...
        {event.image_url ? <img src={event.image_url} alt={event.name} className="absolute inset-0 w-full h-full object-cover" /> : <div className="absolute inset-0 bg-gradient-to-br from-violet-950 to-cyan-950" />}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="max-w-2xl">
            <div className="flex flex-wrap gap-3 mb-4">
              ...
              <span className="px-3 py-1 bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 rounded-full text-sm backdrop-blur-md flex items-center gap-2">
                <MapPin className="w-4 h-4" /> {event.venue || 'Venue announced soon'}
              </span>
            </div>
          ...
          <div className="glass-card p-6 rounded-2xl md:min-w-[300px] text-center shrink-0">
            ...
            <div className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-cyan-400 mb-6">
              {(Number(event.base_price_wei) / 1e18).toString()} ETH
            </div>
            <TransactionButton
        ...
        <div className="md:col-span-2 space-y-8">
          <section>
            ...
            <p className="text-gray-300 leading-relaxed text-lg whitespace-pre-wrap">
              {event.description || 'Event details will be announced by the organizer.'}
            </p>
          </section>
          ...
          <section className="glass-card rounded-2xl p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-sm font-medium text-cyan-400 bg-cyan-400/10 px-3 py-1 rounded-full">
                {event.total_supply - event.tickets_minted} Remaining
              </span>
            </div>
            ...
            <div className="flex justify-between text-sm text-gray-400">
              <span>{event.tickets_minted} Minted</span>
              <span>{event.total_supply} Total Supply</span>
            </div>
          </section>
        </div>
